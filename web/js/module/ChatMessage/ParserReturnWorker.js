...
export default class ParserReturnWorker {
    ...
    constructor(chatDiv,chatSuggestionsWorker) {
        ...
        chatDiv.addEventListener('click', async function(event) {
            if (event.target.classList.contains("copy-bingcat")) {
                ...
                copyTextToClipboard(messageElement.innerText);
            }
            if(event.target.classList.contains("copy-bingcat-markdown")){
                let messageElement = event.target.parentElement.parentElement.getElementsByClassName('textBlock')[0];
                copyTextToClipboard(messageElement.dataset.the_markdown_text);
            }
    ...
    */
    renderCardRequest(message,father){
        ...
        binglink.href = "javascript:window.webapp.browse('" + src + "')"
        document.body.appendChild(binglink)
        binglink.click()
        document.body.removeChild(binglink)
        ...
        iframe.src = src
        div.appendChild(iframe); */
        /* let iframes = window.frames[0];
        let links = iframes.document.querySelectorAll("a");
        // 遍历每个<a>元素
        for (let link of links) {
            // 获取<a>元素的原始href属性
            let href = link.getAttribute("href");
            link.setAttribute("href", "javascript:window.webapp.browse('" + href + "')"); 
        }*/
    }
    ...
    porserTextBlock(body, father) {
        if (!body.size) {
            ...
            for(let i=0;i<aaas.length;i++){
                let link = aaas[i].href;
                aaas[i].href = `javascript:window.webapp.browse('${link}')`
            }
            //如果是注释则加上上标样式
            for(let i=0;i<aaas.length;i++){
            ...
            <div class="copy-bingcat-markdown click">复制md</div>
    ...
    */
    ...
    porserSourceAttributions(sourceAttributions, father) {
        ...
        for (let i = 0; i < sourceAttributions.length; i++) {
            ...
            html = html + `<a href="javascript:window.webapp.browse('${sourceAttribution.seeMoreUrl}');">${sourceAttribution.providerDisplayName}</a>`;
        }
